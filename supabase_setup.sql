...
CREATE TABLE public.messages (
    ...
    room_id BIGINT -- Esta coluna será uma chave estrangeira para chat_rooms
);

-- Ativar RLS para a tabela de mensagens
ALTER TABLE public.messages ENABLE ROW LEVEL SECURITY;

-- Políticas de segurança para a tabela `messages`:
-- Os utilizadores só podem ver as mensagens da sala em que estão.
CREATE POLICY "Users can only see messages in their room."
    ON public.messages FOR SELECT
    USING ( room_id IN (SELECT room_id FROM public.chat_players WHERE player_id = auth.uid()) );

-- Os utilizadores só podem inserir mensagens na sala em que estão.
CREATE POLICY "Users can only insert messages in their room."
    ON public.messages FOR INSERT
    WITH CHECK ( user_id = auth.uid() AND room_id IN (SELECT room_id FROM public.chat_players WHERE player_id = auth.uid()) );
